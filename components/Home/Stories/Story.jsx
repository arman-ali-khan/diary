...
function Story({story}) {
  return (
    <section className="card-container truncate w-full relative">
     ...
     {convertToBengaliNumber(3.5)} <input type="radio" name="rating-3" className="mask text-xs w-4 inline-block mask-star bg-orange-400" defaultChecked />
      ...
      <Link href={`/story/12/123`} className="rounded-t-lg overflow-hidden">
        ...
        <div className="card-body truncate mt-1 !p-0 mb-2 !gap-0">
          <Link className="" href={`/story/12/123`}>
          <h2 style={{lineHeight:'22px',marginTop:'5px'}} data-tip={story?.title} className="card-title w-full overflow-hidden text-ellipsis font-[SolaimanLipi] tooltip flex justify-start text-left text-sm py-0 sm:text-base md:text-lg lg:text-lg !px-1 !m-0">{`${story?.title||'ভালোবাসার মানুষ'}`}</h2>
          </Link>
